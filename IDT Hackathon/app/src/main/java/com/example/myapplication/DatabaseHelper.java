...
//        Log.d(AppConstants.LOG_TAG, "DatabaseHelper onCreate");
        ...
        sBuffer.append("[pid] INTEGER,");
        sBuffer.append("[date] TEXT)");

        // 执行创建表的SQL语句
        db.execSQL(sBuffer.toString());
        StringBuffer ssBuffer = new StringBuffer();
        ssBuffer.append("CREATE TABLE [" + Table_passage + "] (");
        ssBuffer.append("[pid] INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, ");
        ssBuffer.append("[name] TEXT,");
        ssBuffer.append("[date] TEXT)");
        db.execSQL(ssBuffer.toString());
