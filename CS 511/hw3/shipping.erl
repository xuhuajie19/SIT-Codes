...
get_occupied_docks(Shipping_State, Port_ID) ->
    L = [element(2,X) || X<-Shipping_State#shipping_state.ship_locations, element(1,X)==Port_ID].

get_ship_location(Shipping_State, Ship_ID) ->
    {X,Y,_} = lists:keyfind(Ship_ID,3,Shipping_State#shipping_state.ship_locations),
    {X,Y}.
...
get_container_weight(Shipping_State, Container_IDs) ->
    ...
    case T of
        [] -> 
            (lists:keyfind(H,#container.id,Shipping_State#shipping_state.containers))#container.weight;
        _Else -> 
            (lists:keyfind(H,#container.id,Shipping_State#shipping_state.containers))#container.weight+get_container_weight(Shipping_State, T)
    end.

get_ship_weight(Shipping_State, Ship_ID) ->
    {_,R} = maps:find(Ship_ID,Shipping_State#shipping_state.ship_inventory),
    get_container_weight(Shipping_State, R).

load_ship(Shipping_State, Ship_ID, Container_IDs) ->
    {PortA,_,_} = lists:keyfind(Ship_ID,3,Shipping_State#shipping_state.ship_locations),
    
    {_,A} = maps:find(Ship_ID,Shipping_State#shipping_state.ship_inventory),
    TMP1 = maps:update(Ship_ID, lists:merge(A,Container_IDs), Shipping_State#shipping_state.ship_inventory),

    {_,B} = maps:find(PortA,Shipping_State#shipping_state.port_inventory),
    TMP2 = maps:update(PortA, lists:subtract(B,Container_IDs), Shipping_State#shipping_state.port_inventory),

    #shipping_state{ships = Shipping_State#shipping_state.ships, containers = Shipping_State#shipping_state.containers, ports = Shipping_State#shipping_state.ports, ship_locations = Shipping_State#shipping_state.ship_locations, ship_inventory = TMP1, port_inventory = TMP2}.

unload_ship_all(Shipping_State, Ship_ID) ->
    {PortA,_,_} = lists:keyfind(Ship_ID,3,Shipping_State#shipping_state.ship_locations),
    {_,A} = maps:find(Ship_ID,Shipping_State#shipping_state.ship_inventory),
    TMP1 = maps:update(Ship_ID, [], Shipping_State#shipping_state.ship_inventory),
    
    {_,B} = maps:find(PortA,Shipping_State#shipping_state.port_inventory),
    TMP2 = maps:update(PortA, lists:merge(B,A), Shipping_State#shipping_state.port_inventory),
...
unload_ship(Shipping_State, Ship_ID, Container_IDs) ->
    {PortA,_,_} = lists:keyfind(Ship_ID,3,Shipping_State#shipping_state.ship_locations),
    {_,A} = maps:find(Ship_ID,Shipping_State#shipping_state.ship_inventory),
    TMP1 = maps:update(Ship_ID, lists:subtract(A,Container_IDs), Shipping_State#shipping_state.ship_inventory),
    
    {_,B} = maps:find(PortA,Shipping_State#shipping_state.port_inventory),
    TMP2 = maps:update(PortA, lists:merge(B,Container_IDs), Shipping_State#shipping_state.port_inventory),

    #shipping_state{ships = Shipping_State#shipping_state.ships, containers = Shipping_State#shipping_state.containers, ports = Shipping_State#shipping_state.ports, ship_locations = Shipping_State#shipping_state.ship_locations, ship_inventory = TMP1, port_inventory = TMP2}.

set_sail(Shipping_State, Ship_ID, {Port_ID, Dock}) ->
    TMP = lists:keyreplace(Ship_ID, 3, Shipping_State#shipping_state.ship_locations, {Port_ID, Dock, Ship_ID}),

    #shipping_state{ships = Shipping_State#shipping_state.ships, containers = Shipping_State#shipping_state.containers, ports = Shipping_State#shipping_state.ports, ship_locations = TMP, ship_inventory = Shipping_State#shipping_state.ship_inventory, port_inventory = Shipping_State#shipping_state.port_inventory}.
