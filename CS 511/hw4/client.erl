...
listen(State) ->
    receive
        ...
        {From,Ref,connect,History,Name} ->
            {History, NextState} = history_loop(State, History, Name, Ref, From),
            whereis(list_to_atom(State#cl_st.gui))!{result,self(),Ref,History},
            listen(NextState);

        {_From,Ref,ack_leave,ChatName} ->
            NextState=leave_loop(State,Ref,ChatName),
...
%% Preparation :(
history_loop(State, History,Name,_Ref,From) ->
	NewState=#cl_st{gui = State#cl_st.gui, nick = State#cl_st.nick, con_ch = maps:put(Name,From,State#cl_st.con_ch)},
	{History,NewState}.

leave_loop(State,_Ref,ChatName) ->
	NewState=#cl_st{gui = State#cl_st.gui, nick = State#cl_st.nick, con_ch = maps:remove(ChatName, State#cl_st.con_ch)},
	NewState.
...
do_msg_send(State, Ref, ChatName, Message) ->
    ChatPID = element(2, maps:find(ChatName,State#cl_st.con_ch)),
