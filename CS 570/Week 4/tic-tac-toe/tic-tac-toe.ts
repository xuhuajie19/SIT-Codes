...
var board: string[][];
var players: number;
var sizex,sizey: number;
var round: number;
var wincount: number = 3;
var pSymbol: String = "XOABCDEFGHIJKLMNPQRSTUVWYZ";

//IndexOF

function validcoordinate(x,y:number):boolean{
...
function judgerow(x,y:number){
	for (var i=1;i<=sizex-wincount+1;i++){
		...
		if (w){
			displayboard();
			console.log('Player ',board[i][y],' won!');
			process.exit(0);
		} 
	}
}
function judgecolumn(x,y:number){
	for (var i=1;i<sizey-wincount+1;i++){
		...
		if (w){
			displayboard();
			console.log('Player ',board[x][i],' won!');
			process.exit(0);
...
function judgecross(x,y:number){
	...
	for (var i=1;i<=sizey-wincount+1;i++){
		...
		if (w){
			displayboard();
			console.log('Player ',board[x][y],' won!');
			process.exit(0);
		}
	}
...
function ask(x: number){
	...
	switch (command){
		...
		// O 
		case "SAVE":{
			var oname :string = readlinesync.question('What is the filename?');
		...
		case "LOAD":{
			let filename: string= readlinesync.question('What is the filename?');
			var file: string = fs.readFileSync(filename,'utf8','r+');
